...
public class DexElementFuse {
    ...
    public void mainPluginFuse(Context mContext) throws Exception {
        ...
        for (int i = 0; i < newDexElementsLength; i++) {
            // 先融合宿主
            if (i < mainLen) {
                Array.set(newDexElements, i, Array.get(mainDexElements, 0));
    ...
    public void hookActivityResources(Activity activity) {
        try {
            Field mResourcesField = Resources.class.getDeclaredField("mResources");
            mResourcesField.setAccessible(true);
            mResourcesField.set(activity.getResources(), resources);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
